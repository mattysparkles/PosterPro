...
class VineImportService:
    ...
    def create_inventory_records(self, db: Session, *, batch: VineImportBatch, item_ids: list[int], include_locked: bool) -> dict:
        ...
        for item in items:
            ...
            listing = Listing(
                ...
                custom_labels=self._build_labels(item),
                needs_review=True,
                ...
                image_urls=self._lookup_cached_media_urls(db, item.asin),
            )
            db.add(listing)
            db.flush()
    ...
    def create_listing_drafts(self, db: Session, *, batch: VineImportBatch, item_ids: list[int]) -> dict:
        items = db.execute(select(VineImportItem).where(VineImportItem.batch_id == batch.id, VineImportItem.id.in_(item_ids))).scalars().all()
        created = 0
        skipped = 0
        for item in items:
            ...
            db.add(item)
            created += 1
        batch.drafts_created_count = created
        batch.status = "imported" if created else batch.status
        db.add(batch)
        db.commit()
        return {"created": created, "skipped": skipped}
