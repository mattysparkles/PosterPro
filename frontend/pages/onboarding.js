...
const STATUS_COPY = {
  ...
  REAUTH_REQUIRED: ['Reconnect needed', 'Google needs you to approve the connection again. PosterPro keeps the platform OAuth setup managed for you.'],
      ...
      {snapshot?.started && !snapshot?.completed && currentTask ? (
        <div className="mx-auto max-w-4xl space-y-5">
          <section className="rounded-2xl border bg-white p-5 shadow-sm sm:p-7">
            ...
            {currentTask.id === 'ai' && status !== 'CONNECTED' ? <div data-setup-spotlight="ai-mode-choice" className="mt-5 space-y-4"><fieldset className="grid gap-3 md:grid-cols-2"><legend className="sr-only">Choose how PosterPro uses AI</legend><Radio name="ai-mode" value="BYO_OPENAI" selected={aiMode === 'BYO_OPENAI'} onChange={async () => { setAiMode('BYO_OPENAI'); await run(() => chooseOnboardingAiMode('BYO_OPENAI')); }} label="Use my own OpenAI account" description="Available now. Connect your account so PosterPro can identify products, research listings, and write content." /><Radio name="ai-mode" value="POSTERPRO_SPONSORED" selected={aiMode === 'POSTERPRO_SPONSORED'} onChange={async () => { setAiMode('POSTERPRO_SPONSORED'); await run(() => chooseOnboardingAiMode('POSTERPRO_SPONSORED')); }} label="Use PosterPro AI" description="Managed AI is planned for eligible paid plans, but activation is not available on this deployment." /></fieldset>
              ...
              {status === 'CONNECTED' ? <Button disabled={busy} onClick={next}>Continue setup</Button> : <div className="flex flex-wrap items-center gap-3"><Button variant="outline" disabled={busy} onClick={async () => { await run(() => chooseOnboardingAiMode('DISABLED')); await next(); }}>Skip AI for now</Button>{aiMode === 'POSTERPRO_SPONSORED' ? <Button disabled={busy} onClick={next}>Continue setup</Button> : null}</div>}
            </div> : null}
