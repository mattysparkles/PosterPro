...
const STATUS_COPY = {
  CONNECTED: ['Connected', 'PosterPro can use this connection.'],
  ...
  ONLINE: ['Connected', 'Your browser is ready to work with signed-in marketplaces.'],
  CONNECTED_UNVERIFIED: ['Almost ready', 'PosterPro is finishing a quick connection check.'],
  COMING_SOON: ['Coming soon', 'Paid PosterPro plan activation is not live here. No payment or upgrade has been activated.'],
  NOT_CONFIGURED: ['Not connected', 'Connect this service when you are ready.'],
  AUTH_REQUIRED: ['Sign-in needed', 'Sign in normally and PosterPro will update this step automatically.'],
  ...
  READY: ['Ready', 'PosterPro finished setting up this connection.'],
...
export default function OnboardingPage() {
  ...
  const [browserConnectionState, setBrowserConnectionState] = useState('IDLE');
  ...
  useEffect(() => {
    const receiveExtensionPresence = (event) => {
      ...
      if (message.type === 'AUTHORIZED') {
        setBrowserConnectionState('AUTHORIZED');
  ...
  const connectThisBrowser = async () => {
    setBusy(true); setError(''); setBrowserConnectionState('CONNECTING');
    try {
      ...
      setExtensionDetected(true);
      window.setTimeout(() => setBrowserConnectionState((value) => value === 'CONNECTING' ? 'TIMEOUT' : value), 15000);
    } catch (caught) {
      setBrowserConnectionState('ERROR');
  ...
  return (
      ...
      {currentTask?.id === 'browser_extension' && browserConnectionState === 'CONNECTING' ? <div role="status" className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">Connecting this browser… PosterPro is waiting for authorization and the first heartbeat.</div> : null}
      {currentTask?.id === 'browser_extension' && browserConnectionState === 'TIMEOUT' ? <div role="alert" className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><p className="font-semibold">We couldn’t finish connecting this browser.</p><p className="mt-1">Keep this page open, make sure the extension is enabled in this browser, and try the connection once more.</p><Button className="mt-3" size="sm" variant="secondary" disabled={busy} onClick={connectThisBrowser}>Try again</Button></div> : null}
      {currentTask?.id === 'browser_extension' && browserConnectionState === 'ERROR' ? <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-900"><p className="font-semibold">PosterPro couldn’t connect this browser.</p><p className="mt-1">Try again once. If it still fails, open Browser Automation in Settings for the technical connection details.</p><Button className="mt-3" size="sm" variant="secondary" disabled={busy} onClick={connectThisBrowser}>Try again</Button><ActionLink className="ml-3" href="/settings?tab=browser_automation">Open connection help</ActionLink></div> : null}
      ...
      {snapshot?.started && !snapshot?.completed && currentTask ? (
        <div className="mx-auto max-w-4xl space-y-5">
          <section className="rounded-2xl border bg-white p-5 shadow-sm sm:p-7">
            ...
            {currentTask.id === 'choose_marketplaces' ? <div data-setup-spotlight="marketplace-choice" className="mt-5 space-y-4"><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => setSelected(DESTINATIONS.map(([id]) => id))}>Select all</Button><Button variant="outline" size="sm" onClick={() => setSelected([])}>Clear all</Button></div><div className="grid gap-3 sm:grid-cols-2">{DESTINATIONS.map(([id, label]) => { const checked = selected.includes(id); const note = id === 'ebay' ? 'Connect your seller account directly.' : 'Uses the PosterPro browser connection.'; return <Checkbox key={id} checked={checked} onChange={() => setSelected((current) => checked ? current.filter((value) => value !== id) : [...current, id])} label={label} description={note} aria-label={`Set up ${label}`} className={checked ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-200' : ''} />; })}</div><p className="text-sm text-slate-600">Only the marketplaces you select become setup steps. You can change this later.</p><Button disabled={busy} onClick={() => run(() => saveOnboardingSelection(selected))}>Save choices and continue</Button></div> : null}

            {currentTask.id === 'ai' && status === 'CONNECTED' ? <div role="status" className="mt-5 rounded-xl border border-green-300 bg-green-50 p-4"><p className="font-semibold text-green-950">PosterPro AI connected ✓</p><p className="mt-1 text-sm text-green-900">Your existing platform AI connection is ready.</p><Button className="mt-3" disabled={busy} onClick={next}>Continue</Button></div> : null}
            {currentTask.id === 'ai' && status !== 'CONNECTED' ? <div data-setup-spotlight="ai-mode-choice" className="mt-5 space-y-4"><div className="grid gap-3 md:grid-cols-2"><Button type="button" variant="secondary" onClick={async () => { setAiMode('BYO_OPENAI'); await run(() => chooseOnboardingAiMode('BYO_OPENAI')); }} className={`h-auto min-h-[170px] rounded-2xl p-5 text-left ${aiMode === 'BYO_OPENAI' ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-100' : 'border-slate-200 bg-white'}`}><span className="text-xs font-bold uppercase tracking-wide text-blue-700">Available now</span><span className="mt-2 block text-lg font-semibold">Use my own OpenAI account</span><span className="mt-2 block text-sm font-normal leading-6 text-slate-600">You connect your key. OpenAI bills your account directly; PosterPro stores it encrypted and never shows it again.</span><span className="mt-3 block text-xs font-normal text-slate-500">Best for Free plan and users who want direct control.</span></Button><Button type="button" variant="secondary" onClick={async () => { setAiMode('POSTERPRO_SPONSORED'); await run(() => chooseOnboardingAiMode('POSTERPRO_SPONSORED')); }} className={`h-auto min-h-[170px] rounded-2xl p-5 text-left ${aiMode === 'POSTERPRO_SPONSORED' ? 'border-violet-500 bg-violet-50 ring-2 ring-violet-100' : 'border-violet-200 bg-violet-50'}`}><span className="text-xs font-bold uppercase tracking-wide text-violet-700">Paid plan · Coming soon</span><span className="mt-2 block text-lg font-semibold">Use PosterPro AI</span><span className="mt-2 block text-sm font-normal leading-6 text-slate-600">Skip OpenAI setup. PosterPro plans to provide AI on eligible paid plans; paid activation is not available on this deployment.</span><span className="mt-3 block text-xs font-semibold text-violet-800">No payment or premium access will be activated here.</span></Button></div>
              {aiMode === 'BYO_OPENAI' ? <div className="rounded-2xl border border-slate-200 p-5"><h2 className="font-semibold">Connect your OpenAI account</h2><ol className="mt-3 list-decimal space-y-2 pl-5 text-sm leading-6 text-slate-700"><li>Click <b>Open OpenAI API Keys</b> and sign in.</li><li>Choose <b>Create new secret key</b>, name it PosterPro, and copy it.</li><li>Paste it here, then click <b>Test and connect</b>.</li></ol><p className="mt-3 text-sm text-slate-600"><b>API key:</b> a private password that lets PosterPro use your AI account.</p><Button className="mt-4" variant="outline" href="https://platform.openai.com/api-keys" external>Open OpenAI API Keys</Button><label data-setup-spotlight="ai-key" className="mt-4 block text-sm font-medium">Paste your key<Input autoComplete="off" type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} className="mt-1" placeholder="OpenAI secret key" /></label><div className="mt-3 flex flex-wrap gap-2"><Button data-setup-spotlight="ai-test" disabled={busy || apiKey.length < 16} onClick={async () => { const result = await run(() => testOnboardingOpenAiKey(apiKey)); if (result?.state === 'CONNECTED') setApiKey(''); }}>Test and connect</Button><Button variant="outline" onClick={() => setApiKey('')}>Clear key</Button></div></div> : null}
              {aiMode === 'POSTERPRO_SPONSORED' ? <div role="status" className="rounded-xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-950">PosterPro Managed AI is coming soon here. No plan checkout or activation is available on this deployment. You can connect your own OpenAI account or skip AI for now. <ActionLink className="ml-1" href="#plans">View upcoming plan information</ActionLink></div> : null}
            ...
            {['facebook', 'mercari', 'poshmark', 'vinted', 'offerup'].includes(diagnosticMarketplace) ? <section className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-5" aria-label={`${diagnosticMarketplace} connection`}><h2 className="font-semibold text-slate-950">Connect {diagnosticMarketplace}</h2><p className="mt-2 text-sm leading-6 text-slate-700">Open this marketplace and sign in normally. PosterPro will check the connection automatically and never asks for your password.</p><MarketplaceDiagnosticReport marketplace={diagnosticMarketplace} diagnostic={diagnostic} history={diagnosticHistory} running={busy} onRun={runMarketplaceDiagnostic} />{diagnostic?.status === 'LOGIN_REQUIRED' ? <p className="mt-2 text-sm text-amber-900">Sign in to {diagnosticMarketplace} in this same browser; PosterPro will recheck automatically.</p> : null}</section> : null}
            {currentTask.guidance && currentTask.id !== 'browser_extension' && !(currentTask.id === 'google_photos' && ['CONNECTED', 'COMPLETE'].includes(status)) ? <div data-setup-spotlight="task-guidance" className="mt-5 rounded-2xl border border-blue-100 bg-blue-50 p-5"><h2 className="font-semibold text-slate-950">Next action</h2><ol className="mt-3 list-decimal space-y-2 pl-5 text-sm leading-6 text-slate-700">{(currentTask.guidance.steps || []).slice(0, 3).map((step, index) => <li key={`${currentTask.id}-guide-${index}`}>{step}</li>)}</ol>{currentTask.guidance.open_url ? <Button data-setup-spotlight="task-open-button" href={currentTask.guidance.open_url} external>{currentTask.guidance.open_label || 'Open website'}</Button> : currentTask.deep_link ? <Button data-setup-spotlight="task-open-button" href={currentTask.deep_link} variant="secondary">{currentTask.guidance.open_label || 'Open setup'}</Button> : null}<details className="mt-3 text-sm"><summary className="cursor-pointer font-medium text-slate-600">Having trouble?</summary><div className="mt-2 text-slate-600">{currentTask.guidance.troubleshooting || 'You can retry this step from Settings.'}<Button className="ml-2" size="sm" variant="tertiary" disabled={busy} onClick={() => run(() => verifyOnboardingTask(currentTask.id))}>Retry check</Button></div></details></div> : null}
          ...
          <div className="flex flex-wrap justify-between gap-2"><div className="flex gap-2"><Button variant="outline" disabled={stepIndex <= 0 || busy} onClick={() => goToTask(taskList[Math.max(0, stepIndex - 1)].id)}>Back</Button><Button variant="ghost" disabled={busy} onClick={async () => { await run(() => skipOnboardingTask(currentTask.id)); await next(); }}>Skip this step</Button></div><span className="self-center text-xs text-slate-500">Need to leave? You can resume setup from Dashboard or Settings.</span></div>
            <section className="rounded-2xl border border-violet-200 bg-violet-50/60 p-4" aria-label="Ask PosterPro for setup help"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold text-slate-950">Need help? Ask PosterPro</p><p className="mt-1 text-sm text-slate-600">Ask about this step. PosterPro uses the connected AI provider when available; otherwise it gives the saved step-specific guidance.</p></div><Button type="button" variant="outline" onClick={() => { setHelpOpen((open) => !open); setHelpAnswer(null); }}> {helpOpen ? 'Close help' : 'Ask PosterPro'} </Button></div>{helpOpen ? <form className="mt-4 space-y-3" onSubmit={askForHelp}><label className="block text-sm font-medium text-slate-800">What is confusing about “{currentTask.title}”?<Textarea value={helpQuestion} onChange={(event) => setHelpQuestion(event.target.value)} maxLength={1000} rows={3} className="mt-1" placeholder="For example: I do not see the button you mentioned." /></label><Button type="submit" disabled={helpBusy || !helpQuestion.trim()}>{helpBusy ? 'Getting help…' : 'Send question'}</Button>{helpAnswer ? <div role="status" className="rounded-xl border border-violet-200 bg-white p-4 text-sm leading-6 text-slate-800"><p>{helpAnswer.answer}</p><p className="mt-2 text-xs text-slate-500">{helpAnswer.ai_assisted ? 'AI-assisted using this setup step only.' : 'Step-specific saved guidance.'} Chat is not saved. Do not include passwords, API keys, cookies, or tokens.</p></div> : null}</form> : null}</section>
        </div>
      ) : null}
      {snapshot?.completed ? <section className="mx-auto max-w-3xl rounded-2xl border border-green-200 bg-green-50 p-6"><h2 className="text-2xl font-semibold text-green-950">Setup complete</h2><p className="mt-2 text-green-900">Your PosterPro workspace is ready. Here is the current connection summary.</p><div className="mt-5 grid gap-2 sm:grid-cols-2">{taskList.filter((task) => task.id !== 'test_workflow').map((task) => <div key={task.id} className="flex items-center gap-2 rounded-xl bg-white px-3 py-3 text-sm"><span aria-hidden="true" className={['CONNECTED','COMPLETE','READY','ONLINE'].includes(String(task.status).toUpperCase()) ? 'text-emerald-700' : 'text-amber-700'}>{['CONNECTED','COMPLETE','READY','ONLINE'].includes(String(task.status).toUpperCase()) ? '✓' : '!'}</span><span className="font-medium text-slate-800">{task.title}</span></div>)}</div><div className="mt-5 flex flex-wrap gap-2"><Button href="/app">Go to Dashboard</Button><Button href="/settings?tab=marketplaces" variant="secondary">Review connections</Button></div></section> : null}
