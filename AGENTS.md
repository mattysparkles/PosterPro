...
| PP-MARKET-JOB-001 | Durable tenant-scoped assisted job lifecycle | Crosspost parent and extension work were disconnected; some paths could fabricate success | extension models/migration, API, workers, Jobs | Normal publish and crosspost workers now enqueue one shared child job; paired devices claim with leases; CREATE/END transitions persist external identity; unknown post-submit CREATE cannot auto-retry; Jobs exposes child state/details; sale fan-out queues END without prematurely marking DELETED | Automated API/worker tests only; production migration/deployment pending in this tranche | AUTOMATED_TESTED | Apply migration, deploy backend/worker/frontend, exercise live paired browser through review stop |
