...
| PP-FB-001 | Facebook publish/update/sold/delist through canonical adapter | Prior bridge could report a synthetic listing result; actual live form flow unproven | connector, durable extension jobs, browser adapter, Jobs/Settings | Durable CREATE and human-confirmed END use tenant-scoped extension jobs; mock Facebook DOM test fills title/price/description/photos and stops at AWAITING_OPERATOR_REVIEW; UPDATE and automated sale polling remain unsupported | No authenticated Facebook browser E2E; no listing submitted | PARTIAL | Verify selectors/session in operator browser; implement destination taxonomy/condition mapping and exact-listing UPDATE |
| PP-MERCARI-001 | Mercari <=1000-char projection and lifecycle | Canonical 1000-character description projection exists; marketplace UI selectors unverified | connector, mapper, extension transport | Durable CREATE/END transport, lease and tenant tests; form selectors only cover generic title/price/description/photos fixture contract | Not live verified; no authorized seller-session E2E | PARTIAL | Verify Mercari form selectors; implement required category/condition/shipping mapping and UPDATE |
| PP-POSH-001 | Poshmark market-specific handoff/lifecycle | Payload projection exists; DOM flow unverified | connector, mapper, extension transport | Durable CREATE/END transport is shared; current content adapter intentionally leaves category/condition for operator | Not live verified | PARTIAL | Validate Poshmark selectors, size/category translation, seller-session flow, and UPDATE |
| PP-VINTED-001 | Vinted market-specific handoff/lifecycle | Prior provider-assist path did not enter the shared extension queue | connector, worker, extension transport | `provider_assist` now enters the shared durable queue; shared CREATE/END paths tested | Not live verified | PARTIAL | Validate Vinted country/domain and form selectors, parcel mapping, and UPDATE |
| PP-ETSY-001 | Etsy API-first listing lifecycle | No direct Etsy API connector/OAuth flow exists; fallback adapter only | connector, OAuth/API, extension transport | Truthful capability currently exposes assisted CREATE/END only; shared safe-review transport exists | AUTH_REQUIRED / not live verified | PARTIAL | Implement Etsy OAuth/API if app authorization is available; otherwise verify assisted selectors and disclose review-required fields |
| PP-OFFERUP-001 | OfferUp assisted listing lifecycle | Connector/marketplace enum scaffold added; destination form not verified | connector, mapper, extension transport | Shared durable CREATE/END contract and hostname guard apply | Not live verified | PARTIAL | Validate current OfferUp host and seller form; map local/shipping/category/condition; implement UPDATE |
| PP-EXT-001 | Installable, paired browser extension with secure tenant/device queue | Prior ZIP/pairing surfaced controls but no durable queue integration | browser-extension, bridge, extension API, Settings/Jobs UI | Dynamic ZIP endpoint; one-time authenticated-user pairing; hashed device token; heartbeat/version gate; exclusive leased claim/recovery; unknown CREATE outcome blocks duplicate retry; safe create-form test reaches review without submission; cookie/localStorage/sessionStorage values are no longer exported | Deployed ZIP endpoint returned HTTP 200; backend contract tests pass; no operator Chromium install or marketplace session E2E | PARTIAL | Load ZIP in operator browser, pair, prove heartbeat/claim on live UI; site selectors/auth remain externally unverified |
| PP-MARKET-001 | Shared explicit marketplace capability contract | Previous fallback connectors implied publish parity | connectors/capabilities.py, registry, field mapper | Direct eBay and assisted capability flags are explicit; assisted UPDATE is not advertised; Etsy remains AUTH_REQUIRED for direct API | Source + automated connector tests only | AUTOMATED_TESTED | Add destination-level taxonomy/condition validation and live account checks |
| PP-MARKET-JOB-001 | Durable tenant-scoped assisted job lifecycle | Crosspost parent and extension work were disconnected; some paths could fabricate success | extension models/migration, API, workers, Jobs | Normal publish and crosspost workers now enqueue one shared child job; paired devices claim with leases; CREATE/END transitions persist external identity; unknown post-submit CREATE cannot auto-retry; Jobs exposes child state/details; sale fan-out queues END without prematurely marking DELETED; failed update action preserves Published listing lifecycle | 50 focused marketplace/eBay tests passed; scoped migration applied; backend/worker/frontend restarted; health and local/public Jobs routes return 200 | OPERATOR_TEST_REQUIRED | Pair the current ZIP in an operator browser and walk a safe assisted job to AWAITING_OPERATOR_REVIEW |
